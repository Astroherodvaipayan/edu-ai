...
from auth import router as auth_router, signup_user, login_user
            ...
            try:
                ...
                with yt_dlp.YoutubeDL() as ydl:
                    ...
                    if info.get('subtitles'):
                        full_text = info['subtitles'].get('en', [{}])[0].get('text', '')
                    else:
...
@app.post("/signup")
async def signup_endpoint(auth: AuthRequest):
    # HTTPException will be raised from signup_user if there's an error
    result = await signup_user(auth.email, auth.password)
    # Supabase signup might not return an 'error' field in the JSON for all errors,
    # it might return a non-2xx status code with a message.
    # The error handling is now primarily within signup_user.
    return {"message": "User registration process initiated.", "data": result}

@app.post("/login")
async def login_endpoint(auth: AuthRequest):
    # HTTPException will be raised from login_user if there's an error
    result = await login_user(auth.email, auth.password)
    # Error handling is now primarily within login_user.
    return {"access_token": result.get("access_token"), "token_type": "bearer", "user": result.get("user")}
